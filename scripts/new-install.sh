...
if [ $OS_ID = 'fedora' ]; then
    ...
    dnf install -y --skip-broken git curl pip wget; service wazuh-agent status
elif [ $OS_ID = 'debian' ]; then
    echo "$OS_ID"
    apt-get update -y 
    apt-get install -y --fix-broken git curl pip wget; systemctl status wazuh-agent
elif [ $OS_ID = 'alpine' ]; then
    echo "$OS_ID"
    apk add -y git curl pip wget; systemctl status wazuh-agent
